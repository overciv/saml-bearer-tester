...
async function createApp({ oktaDomain, adminApiToken, label, applicationType, tokenEndpointAuthMethod, redirectUris, postLogoutUris, grantTypes, scopes }) {
  const responseTypes = [];
  if (grantTypes?.includes('authorization_code')) responseTypes.push('code');
  if (grantTypes?.includes('implicit'))           responseTypes.push('token', 'id_token');
...
module.exports = { createApp, getApp, cloneApp, findUser, listFactors, resetFactor, getSystemLog };
